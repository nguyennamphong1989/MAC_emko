...
uint8_t period0,period1,period4,period5,period6,period7;
uint8_t state0=0;
uint8_t state1=0;
uint8_t state4=0;
uint8_t state5=0;
...
static void r_Config_ICU_irq0_interrupt(void)
{
	...
	if(state0==1) // wait for second edge
	{
		timestamp0[1] = tick;
...
static void r_Config_ICU_irq1_interrupt(void)
{
	...
	if(state1==1) // wait for second edge
	{
		timestamp1[1] = tick;
...
static void r_Config_ICU_irq4_interrupt(void)
{
	...
	if(state4==1) // wait for second edge
	{
		timestamp4[1] = tick;
