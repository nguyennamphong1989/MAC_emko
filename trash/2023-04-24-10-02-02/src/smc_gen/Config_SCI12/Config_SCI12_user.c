...
uint16_t UART12_timecount; // add to receive interrupt
uint8_t UART12_state=0;
...
MD_STATUS R_SCI12_AsyncTransmit (uint8_t * const tx_buf,uint16_t tx_num)
{
	 ...
	 status = R_Config_SCI12_Serial_Send(tx_buf, tx_num);
	 /* Wait for transmit end flag */
	 while (0U == SCI12_txdone)
	 {
		 /* Wait */
	 }
	 return (status);
